...
/*	rudp_obj = rudp_init("server", 0x4444, 0x4242, 1);
	...
	tick = SDL_GetTicks();
	cc = 0;*/
	sock_in = SDLNet_UDP_Open(0x4242);
	pack_in = SDLNet_AllocPacket(42);
	...
	{/*
		...
		if (SDLNet_UDP_Recv(sock_in, pack_in) > 0)
			printf("got: '%s'\n", pack_in->data);
	}
	SDLNet_UDP_Close(sock_in);
	SDLNet_FreePacket(pack_in);
//	rudp_close(rudp_obj);
	return (0);
}
...
int	client(void *data)
{
	...
	ip = &((char*)data)[1];
	//
	sock_out = SDLNet_UDP_Open(0);
	pack_out = SDLNet_AllocPacket(5);
	SDLNet_ResolveHost(&pack_out->address, ip, 0x4242);
	pack_out->len = 5;
	SDL_memcpy(pack_out->data, "test", 5);
	//
/*	rudp_obj = rudp_init("client", 0x4242, 0x4444, 1);
	...
	{
		SDLNet_UDP_Send(sock_out, -1, pack_out);
	}
	SDLNet_UDP_Close(sock_out);
